...
export default async function WorkDashboard({ searchParams }: Props) {
  ...
  const requestedView = parseView(first(params.view));
  ...
  return (
    <div className="admin-stack">
      <div>
        ...
        <p className="lede">Coordinate collaborators across Saints, Traditions, Places, and Instagram posts. Completing an assignment records workflow progress; it never publishes content.</p>
      </div>
      ...
      {view === "mine" ? <AssignmentSection title="My work" empty="You have no active assignments." rows={mine} refs={refs} users={users} userId={user.id} canManage={canManage} /> : null}
      {view === "available" ? <AssignmentSection title="Available work" empty="No work is currently available for self-assignment." rows={available} refs={refs} users={users} userId={user.id} canManage={canManage} available={canClaim} readOnly={!canClaim} /> : null}
      {view === "blocked" ? <AssignmentSection title="Blocked" empty="No visible assignments are blocked." rows={blocked} refs={refs} users={users} userId={user.id} canManage={canManage} /> : null}
      {view === "completed" ? <AssignmentSection title="Recently completed" empty="No assignments have been completed yet." rows={completed} refs={refs} users={users} userId={user.id} canManage={canManage} readOnly /> : null}
...
function AssignmentSection({ title, empty, rows, refs, users, userId, canManage, available = false, readOnly = false }: {
  ...
  return (
    <section>
      <div className="section-heading"><h2>{title}</h2><StatusBadge label={String(rows.length)} /></div>
      {rows.length ? (
            ...
            return (
              <article className="review-row" key={row.id}>
                <div className="review-row__content">
                  ...
                  <h3>{ref ? <Link href={ref.href}>{ref.label}</Link> : "Missing content record"}</h3>
...
function TeamWorkload({ assignments, users }: { assignments: AssignmentRow[]; users: AdminUser[] }) {
  ...
  return (
    <section>
      <div className="section-heading"><h2>Team workload</h2><StatusBadge label={`${activeRows.length} active`} /></div>
      <div className="review-list">
        {users.map((user) => {
          ...
          return (
            <div className="review-row" key={user.id}>
              <div><h3>{user.name || user.email}</h3>{user.name ? <p>{user.email}</p> : null}</div>
