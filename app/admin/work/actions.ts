...
function fail(message: string): never { redirect(`/admin/work?error=${encodeURIComponent(message)}`); }
function done(value: string): never { revalidatePath("/admin"); revalidatePath("/admin/work"); redirect(`/admin/work?updated=${value}`); }
