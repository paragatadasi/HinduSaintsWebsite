...
Configure at least one admin sign-in method:

- Passwordless email: `AUTH_RESEND_KEY` and `AUTH_EMAIL_FROM`
- Google OAuth: `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`

Also configure the initial Site Admin bootstrap list:
...
`ADMIN_EMAIL_ALLOWLIST` is a comma-separated bootstrap list used only when no active Site Admin exists. After bootstrap, manage access through **Users & Access**. Passwordless email works with any mailbox provider; its sender domain must be verified in Resend. Missing credentials hide the corresponding sign-in method, and a stale dev server may require a restart after environment changes.
