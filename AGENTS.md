...
- Git commands that push to `origin` may also trigger the app's external-transfer approval because they send repository history to GitHub. Do not stop with a prose question when the user has already asked to prepare, queue, or deploy; request the normal tool approval immediately with a clear justification and then continue the workflow after approval.
...
- When the user tells an individual feature agent to "queue for deployment" or simply `q`, do the same preparation steps as "prepare for deployment", but run `npm run queue:deployment` or otherwise mark the handoff status as `queued` instead of `ready`. Queued handoffs mean the branch is deployable but intentionally waiting to be bundled with the next major, user-requested, or release-captain-triggered deployment. The feature agent should notify the active release-captain task with the same branch, SHA, handoff, verification, migration, environment, risk, and deploy-note details, and clearly state that the handoff is queued rather than requesting immediate deployment.
- The instructions "prepare for deployment", "queue for deployment", and `q` include user intent to push the scoped feature branch and handoff commit to `origin` after verification. They do not authorize merging to `main` or `deploy`. If Codex asks for explicit external-transfer approval for that feature-branch push, use the approval flow rather than asking the user to restate permission in a separate message.
...
- For integrated release verification, prefer a persistent release verification worktree prepared with `npm run release:worktree` over creating a fresh dependency install for every release. This worktree must keep a real `node_modules` directory, not a junction/symlink to another install, because Windows junctions can break Next.js standalone trace copying.
- The persistent release verification install must include dev dependencies. Do not use a production-only install for release verification: `dev:check`, tests, Prisma generation, TypeScript validation, and `codex:verify` require dev tools such as `prisma`, `typescript`, and `tsx`. Production-only dependency pruning belongs to the deployed runtime image, not the release-captain verification worktree.
